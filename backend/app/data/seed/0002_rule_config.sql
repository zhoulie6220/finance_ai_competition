...
('valuation.terminal_growth_max', '0.02', 'percent', '',
 '终值增长率上限',
 '⬜ 待会计确认（问题 6）｜周期行业不得给高永续增速，上限取长期通胀水平。方向有共识，但 0.02 这个具体数字是实现时拍的。',
 '比例', '0.02', '0.00', '0.03'),

('valuation.wacc_min', '0.06', 'percent', '',
 'WACC 下限',
 '⬜ 待会计确认（问题 6）｜防止压力情景下 WACC 被推到不切实际的水平。方向有共识，但 0.06 这个具体数字是实现时拍的。',
 '比例', '0.06', '0.03', '0.15'),
...
('index.direction.weak_ratio', '0.5', 'percent', '',
 '部分支持的幅度分界',
 '⬜ 待会计确认（问题 5）｜实际幅度低于主张目标幅度（或无目标时取历史同类主张中位幅度）的该比例时，判为部分支持。方案选择 §10 只说了「幅度较弱判部分支持」，没给比例，0.5 是实现时拍的。',
 '比例', '0.5', '0.2', '0.8'),
...
--   全部为 `⬜ 待会计确认`，见 `待会计确认.md` 问题 5。
('index.weight.supported', '1.0', 'percent', '', '支持观测的计分权重',
 '⬜ 待会计确认（问题 5）｜各状态在指数分子中的权重。A-7 未给状态权重，1.0 是实现时拍的。',
 '倍', '1.0', '0.5', '1.5'),
('index.weight.partial', '0.5', 'percent', '', '部分支持观测的计分权重',
 '⬜ 待会计确认（问题 5）｜部分支持按半权计入。A-7 未给，0.5 是实现时拍的。',
 '倍', '0.5', '0.0', '1.0'),
('index.weight.conflicted', '-1.0', 'percent', '', '冲突观测的计分权重',
 '⬜ 待会计确认（问题 5）｜冲突为负权重，负多重等于「一次冲突扣多少分」。A-7 未给，−1.0 是实现时拍的。',
 '倍', '-1.0', '-1.5', '-0.5'),
