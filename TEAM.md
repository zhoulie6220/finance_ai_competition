...
**还欠 K 的：完整清单在仓库根目录的 `待会计确认.md`**（A-1 … A-6），
那份文件是唯一真源，K 答完由计算机同学落到代码/种子里。摘要：

| 条目 | 是什么 | 为什么必须 K 定 |
|---|---|---|
| A-1 🔴 | 正负号口径 | `资产减值损失` 2015 印正数、2024 印负数。`sign_convention` 取值域在 schema 里，但**没有任何文档说哪些科目标哪个值**。不统一的话跨年比大小会把 `+3亿` 和 `−3亿` 当成方向相反 |
| A-2 🔴 | 主题→指标映射 | 「降本增效 → 验毛利率」这类映射是实现时定的。**指标选错，「叙事与事实相悖」就是误报** |
| A-3 🟡 | 「产能释放」算不算利好 | 2024 年报里它是利空（供给过剩）。当前刻意不收，需确认 |
| A-4 🔴 | 情景传导的整套数字 | `[0.60,0.25,0.15]` 等。五份 Word 对传导**只描述方向，一个数字都没有** |
| A-5 🟡 | 观测状态的计分权重 | `1.0 / 0.5 / −1.0` 与 `weak_ratio = 0.5`。**冲突的负权重定多重 = 一次冲突扣多少分** |
| A-6 🟡 | 估值上下限 | `terminal_growth_max = 0.02`、`wacc_min = 0.06` |
| B-1 | 删掉幅度阈值后的后果 | 宝钢 2020 那两条 0.04 个百分点的被判成冲突了，冲突数 8 → 10 |

另有 `docs/05-assumptions-and-risks.md`（假设与风险，比赛明确要求）与审 `docs/06`。

> A-4 / A-5 / A-6 **目前正在生效**（`rule_config` 里全是 `tier='hard'`，共 12 条
